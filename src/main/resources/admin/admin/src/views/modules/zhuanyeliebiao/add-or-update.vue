<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="专业类型" prop="zhuanyeleixing">
          <el-select v-model="ruleForm.zhuanyeleixing" placeholder="请选择专业类型">
            <el-option
                v-for="(item,index) in zhuanyeleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="专业类型" prop="zhuanyeleixing">
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="专业详情" prop="zhuanyexiangqing">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.zhuanyexiangqing" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.zhuanyexiangqing" label="专业详情" prop="zhuanyexiangqing">
                    <span v-html="ruleForm.zhuanyexiangqing"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	zhuanyexiangqing : false,
	zhuanyeleixing : false,
      },
      ruleForm: {
        zhuanyemingcheng: '',
        zhuanyexiangqing: '',
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhuanyemingcheng = true;
          ...
          if(o=='zhuanyexiangqing'){
            this.ruleForm.zhuanyexiangqing = obj[o];
	    this.ro.zhuanyexiangqing = true;
            continue;
          }
          if(o=='zhuanyeleixing'){
            this.ruleForm.zhuanyeleixing = obj[o];
	    this.ro.zhuanyeleixing = true;
            continue;
          }
        }
      }
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.zhuanyeleixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.zhuanyexiangqing = this.ruleForm.zhuanyexiangqing.replace(reg,'../../../springbootsu39z/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhuanyeliebiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhuanyeliebiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
