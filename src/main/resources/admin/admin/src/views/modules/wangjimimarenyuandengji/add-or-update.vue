<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="新密码" prop="xinmima">
          <el-input v-model="ruleForm.xinmima" 
              placeholder="新密码" clearable  :readonly="ro.xinmima"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="新密码" prop="xinmima">
              <el-input v-model="ruleForm.xinmima" 
                placeholder="新密码" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shouji = true;
          ...
          if(o=='xinmima'){
            this.ruleForm.xinmima = obj[o];
	    this.ro.xinmima = true;
            continue;
          }
          if(o=='xiugaishijian'){
    ...
    info(id) {
      this.$http({
        url: `wangjimimarenyuandengji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wangjimimarenyuandengji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wangjimimarenyuandengjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
